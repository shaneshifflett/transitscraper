...
insert_eta = "INSERT INTO eta_eta (name, direction, seconds, minutes, is_departure, affected_by_layover, route_id, vehicle_id, created)\
    ...
    for filename in filenames:
        with open(path+filename, 'rb') as f:
            ...
            for idx, row in enumerate(reader[1:]):
                ...
                stopid = get_stop_id(cur, row[6], routeid)
                seconds = row[2]
                is_departure = row[3]
                dir_tag = row[4]
                minutes = row[7]
                affected_by_layover = row[8]
                triptag = row[1]
                insert_str = cur.mogrify(insert_eta, (triptag, dir_tag, seconds, minutes, is_departure, affected_by_layover, routeid, vehicleid, datetime.now()))
                try:
